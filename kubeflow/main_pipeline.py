...
def training_pipeline(
    ...
    fetch_model_task = fetch_model(
        model_name=huggingface_repo, version=version, hyperparameters=hyperparameters
    )
    kubernetes.use_secret_as_env(
        fetch_model_task,
    ...
    unzip_dataset_task = unzip_dataset(
        data_dir=data_mount_path, dataset=fetch_dataset_task.outputs["dataset"]
    )
    ...
    unzip_model_task = unzip_model(
        data_dir=data_mount_path, model=fetch_model_task.outputs["original_model"]
    )
    ...
    train_model_task = train_model(
        original_model=fetch_model_task.outputs["original_model"],
    ...
    # convert to onnx
    convert_onnx_task = convert_model(
